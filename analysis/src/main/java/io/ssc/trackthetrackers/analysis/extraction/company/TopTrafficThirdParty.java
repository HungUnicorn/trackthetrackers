...
public class TopTrafficThirdParty {
	...
	private static String argPathToTrackingArcs = Config.get("analysis.results.path") + "longArc";
	...
	public static void main(String args[]) throws Exception {
		...
		DataSet<Tuple2<String, Double>> nodeAndValue = ReaderUtils.readNodeValue(env, argPathToNodesAndValues);
		...
		// Output 1, ID, value
		DataSet<Tuple3<Long, Long, Double>> topKMapper = trackerIDAndValue.flatMap(new TopKMapper());
		...
		DataSet<Tuple2<String, Double>> topKwithName = topKReducer.join(thirdPartyIndex).where(1).equalTo(1).projectSecond(0).projectFirst(2);
	...
	public static class NodeReader implements FlatMapFunction<String, Tuple2<String, Long>> {
	...
	public static class ValueReader implements FlatMapFunction<String, Tuple2<String, Double>> {
	...
	public static class ProjectIDAndValues implements FlatMapFunction<Tuple2<Tuple2<String, Double>, Tuple2<String, Long>>, Tuple2<Long, Double>> {

		@Override
		public void flatMap(Tuple2<Tuple2<String, Double>, Tuple2<String, Long>> joinTuple, Collector<Tuple2<Long, Double>> collector)
				throws Exception {
			Long id = joinTuple.f1.f1;
			Double value = joinTuple.f0.f1;
			collector.collect(new Tuple2<Long, Double>(id, value));
		}
	}

	public static class ProjectArcValues implements FlatMapFunction<Tuple2<Tuple2<String, Long>, Tuple2<Long, Double>>, Tuple2<String, Double>> {

		@Override
		public void flatMap(Tuple2<Tuple2<String, Long>, Tuple2<Long, Double>> joinTuple, Collector<Tuple2<String, Double>> collector)
				throws Exception {
			String thirdParty = joinTuple.f0.f0;
			Double value = joinTuple.f1.f1;
			collector.collect(new Tuple2<String, Double>(thirdParty, value));
		}
	}
	...
	public static class ProjectIDWithName implements
