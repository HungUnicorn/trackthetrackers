...
public class WeightedPageRankMain implements ProgramDescription {
	...
	public static void main(String[] args) throws Exception {
		...
		DataSet<Tuple2<Long, Double>> sumEdgeWeight = weightedEdges.groupBy(0).reduceGroup(new SourceSumEdgeWeight());

		// Normalization:sum of weight is equals to one
		DataSet<Tuple3<Long, Long, Double>> normalizedEdgeWeight = weightedEdges.map(new WeightNormalized()).withBroadcastSet(sumEdgeWeight,
				"sumEdgeWeight");

		DataSet<Tuple1<Long>> nodes = weightedEdges.<Tuple1<Long>> project(0).distinct();

		// Get the total count of nodes
		DataSet<Long> numNodes = nodes.reduceGroup(new CountNodes());

		// Initial rank : 1 / numNodes
		DataSet<Tuple2<Long, Double>> pagesRanked = nodes.map(new InitialRanking()).withBroadcastSet(numNodes, "numNodes");

		Graph<Long, Double, Double> network = Graph.fromTupleDataSet(pagesRanked, normalizedEdgeWeight, env);

		DataSet<Vertex<Long, Double>> pageRanks = network.run(new WeightedPageRank<Long>(nodes.count(), DAMPENING_FACTOR, maxIterations))
				.getVertices();
	...
	public static class SourceSumEdgeWeight implements GroupReduceFunction<Tuple3<Long, Long, Double>, Tuple2<Long, Double>> {
		@Override
		public void reduce(Iterable<Tuple3<Long, Long, Double>> edges, Collector<Tuple2<Long, Double>> collector) throws Exception {
	...
	// Use Log?
	public static class WeightNormalized extends RichMapFunction<Tuple3<Long, Long, Double>, Tuple3<Long, Long, Double>> {
		private HashMap<Long, Double> sumEdgeWeightMap = new HashMap<Long, Double>();

		@Override
		public void open(Configuration parameters) throws Exception {

			ArrayList<Tuple2<Long, Double>> sumEdgeWeight = (ArrayList) getRuntimeContext().getBroadcastVariable("sumEdgeWeight");
		...
		@Override
		public Tuple3<Long, Long, Double> map(Tuple3<Long, Long, Double> edge) throws Exception {
			Long node = edge.f0;
			...
			return new Tuple3<Long, Long, Double>(node, connectNode, newEdgeWeight);
		}
	}

	public static class CountNodes implements GroupReduceFunction<Tuple1<Long>, Long> {
		@Override
	...
	public static class InitialRanking extends RichMapFunction<Tuple1<Long>, Tuple2<Long, Double>> {
		...
		public void open(Configuration parameters) throws Exception {
			...
			numNodes = getRuntimeContext().<Long> getBroadcastVariable("numNodes").get(0);
		}
